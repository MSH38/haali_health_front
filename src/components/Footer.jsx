...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-navy-100 bg-surface text-navy-800/70">
      <div className="container-x py-14 lg:py-16">
        <div className="grid gap-10 lg:grid-cols-[1.4fr_repeat(3,1fr)] lg:gap-12">
          {/* Brand */}
          <div>
            <Link to="/" className="inline-block" aria-label="Haali Health">
              <BrandLogo className="h-6" />
            </Link>
            <p className="mt-5 max-w-xs text-sm leading-relaxed text-navy-800/60">
              {t('footer.tagline')}
            </p>
            <div className="mt-6 flex items-center gap-3">
              <a
                ...
                aria-label="Email"
                className="grid h-10 w-10 place-items-center rounded-xl border border-navy-800/15 text-navy-800/60 transition-colors hover:border-mint-600/60 hover:text-mint-700"
              >
          ...
          {columns.map((col, ci) => (
            <div key={col.title}>
              <h3 className="text-sm font-bold text-navy-900">{col.title}</h3>
              <ul className="mt-4 space-y-2.5">
                {col.links.map((link, li) => (
                  <li key={link}>
                    <Link
                      to={COLUMN_HREFS[ci]?.[li] ?? '/'}
                      className="text-sm text-navy-800/65 transition-colors hover:text-mint-700"
                    >
                      {link}
                    </Link>
                  </li>
        ...
        */}
        ...
        {/* Bottom bar */}
        <div className=" flex flex-col mt-2 border-t-2 border-navy-100 items-center justify-between gap-4 sm:flex-row">
          <p className="text-xs mt-5 text-navy-800/50">
            © {year} Haali Health. {t('footer.rights')} • {t('footer.built')}
          </p>
          <div className="flex items-center gap-5">
            <Link
              to="/privacy"
              className="text-xs text-navy-800/50 transition-colors hover:text-mint-700"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 text-xs font-semibold text-navy-800/60 transition-colors hover:text-mint-700"
            >
